...
} from "../../src/lib/matches/providers/juheProvider.js";
...
async function syncJuheWorldCupMatches(request) {
  const fetchedAt = new Date().toISOString();
  const forceSync = new URL(request.url).searchParams.get("force") === "1";
  const cooldownMinutes = readCooldownMinutes(process.env.JUHE_SYNC_COOLDOWN_MINUTES);
  ...
  try {
    ...
    const providerResult = await fetchJuheWorldCupMatches({ env: process.env });
